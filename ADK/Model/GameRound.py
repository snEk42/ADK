...
# PUBLIC METHODS
    ...
    def __init__(self, players, playArea, timer):
    ...
    def NextState(self, game):
        if self.progress == "roundOver" and self.GetMaxScore() < self.targetScore:
            self.playArea.Clear()
            self.PreparePlayersForNextRound()
            pub.sendMessage("STATE CHANGED", state=self.__class__.__name__, scores=self.GetScores())
            self.StartGameRound()
        elif self.progress == "roundOver":
            game.ChangeState( FinalScoreBoard(self.GetScores()) )
...
# PRIVATE METHODS    
    ...
    def StartGameRound(self):
        self.progress = "roundInAction"
        self.ShowPlayers()
        self.timer.Start(0.0005)
